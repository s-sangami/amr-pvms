...
def on_startup():
    try:
        ...
        # Migration: add verified_at/dispensed_at to existing prescriptions table
...
def get_my_prescriptions_detailed(
    ...
    for p in prescriptions:
        ...
        result.append({
            ...
            "is_antibiotic": not_recommended or (aware is not None),
        })
    return result
...
def issue_prescription(payload: PrescriptionCreate, db: Session = Depends(get_db)):
    ...
    # #1 — Refill/overuse check: same drug, still active, not yet dispensed
    overuse_warning = None
    existing = db.query(models.Prescription).filter(
        models.Prescription.patient_abha == payload.patient_abha,
        models.Prescription.drug_name.ilike(payload.drug_name.strip()),
        ~models.Prescription.status.ilike('%dispensed%'),
    ).order_by(models.Prescription.created_at.desc()).first()

    if existing and existing.created_at:
        try:
            course_days = int(existing.duration_days) if existing.duration_days else 0
        except (ValueError, TypeError):
            course_days = 0
        course_end = existing.created_at + timedelta(days=course_days)
        if datetime.utcnow() < course_end:
            overuse_warning = (
                f"Patient still within active course of {existing.drug_name} "
                f"(issued {existing.created_at.date()}, course ends {course_end.date()}). "
                f"Possible overuse — review before prescribing again."
            )
    ...
    if overuse_warning:
        response["overuse_warning"] = overuse_warning
    return response
...
def verify_prescription(prescription_id: str, db: Session = Depends(get_db)):
    ...
    # #4 — record first verification timestamp (don't overwrite on repeat scans)
    if not prescription.verified_at:
    ...
    return {
        ...
        "verified_date": prescription.verified_at.isoformat() if prescription.verified_at else None,
...
def dispense_prescription(prescription_id: str, delivery_method: str = "Physical", db: Session = Depends(get_db)):
    ...
    return {
        ...
        "drug_name": prescription.drug_name,
        "status": prescription.status,
        "dispensed_date": prescription.dispensed_at.isoformat(),
    }
